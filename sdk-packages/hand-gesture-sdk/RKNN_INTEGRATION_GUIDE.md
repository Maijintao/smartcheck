...
### 5.0 MAC 白名单授权（SDK 内置）

`hand-sdk` 已内置 MAC 白名单实时校验逻辑，`HandDetector.init(...)` 在加载模型前会先调用授权服务器：

- 默认服务器：`http://112.74.39.40:8080`
- 授权接口：`POST /api/device/activate`
- 请求体：`{"deviceMac":"AA:BB:CC:DD:EE:FF"}`
...
`HandDetector.init(...)` 返回值：

- `0`：初始化成功
- `-1`：模型或 native 初始化失败
- `-2`：MAC 授权失败（未入白名单或网络异常）

若你在离线调试阶段需要临时关闭授权，可设置：

```kotlin
HandSdkAuth.configure(serverUrl = "http://your-server:8080", required = false)
```
...
```kotlin
HandSdkAuth.configure(serverUrl = "http://112.74.39.40:8080", required = true)

val ret = HandDetector.init(context)
if (ret == HandDetector.INIT_OK) {
    Log.d("App", "HandDetector initialized successfully")
} else if (ret == HandDetector.INIT_AUTH_FAILED) {
    Log.e("App", "Device MAC is not authorized")
} else {
