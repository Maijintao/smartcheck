...
object HandDetector {
     ...
     * @return 0=成功, -1=初始化失败, -2=MAC 授权失败
     */
    fun init(context: Context, licenseKey: String = ""): Int {
        ...
        try {
            val authResult = HandSdkAuth.verifyDeviceAccess(context)
            if (authResult.isFailure) {
                val msg = authResult.exceptionOrNull()?.message ?: "设备未授权"
                Log.e(TAG, "MAC 授权校验失败: $msg")
                return INIT_AUTH_FAILED
            }

            HandSdkAuth.getCurrentDeviceMac(context)?.let { mac ->
                HandSdkAuth.saveVerification(context, mac)
            }
